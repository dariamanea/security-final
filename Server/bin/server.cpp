...
// change the server certificate name from server-certificate.pem to intermediate CERT 
    ...
    // if (SSL_CTX_use_certificate_file(ctx.get(), "ca/intermediate/certs/server.cert.pem", SSL_FILETYPE_PEM) <= 0) {
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx.get(), "server-private-key.pem", SSL_FILETYPE_PEM) <= 0) {
        // if (SSL_CTX_use_PrivateKey_file(ctx.get(), "ca/intermediate/private/server.key.pem", SSL_FILETYPE_PEM) <= 0) {
