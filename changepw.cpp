...
void send_http_request(BIO *bio, const std::string& line, const std::string& host, const std::string& name, const std::string& pass, const std::string& newpass)
{
    std::string request = line + "\r\n";
    request += "Host: " + host + "\r\n";
    request += "\r\n";
    request += std::string("newpass") + "\r\n";
    request += name + "\r\n";
    request += pass + "\r\n";
    request += newpass + "\r\n";
...
int main(int argc, char *argv[])
{
    ...
    std::string newpass = getpass("Enter new password: ");
#if OPENSSL_VERSION_NUMBER < 0x10100000L
    SSL_library_init();
    ...
    my::send_http_request(ssl_bio.get(), "GET / HTTP/1.1", "duckduckgo.com", name, pass, newpass);
